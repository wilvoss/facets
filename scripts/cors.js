...
const upstream = 'http://tinyurl.com';
...
const upstream_path = '/api-create.php?url=';
...
const http_response_headers_set = {
  ...
  'X-Frame-Options': 'ALLOW FROM https://facets.bigtentgames.com', // IE
  'Content-Security-Policy': "frame-ancestors 'self' https://facets.bigtentgames.com;", // Chrome, Firefox, etc.
...
async function fetchAndApply(request) {
  ...
  if (blocked_regions.includes(region) || blocked_ip_addresses.includes(ip_address)) {
    ...
    if (method.toUpperCase() === 'POST' && request_content_type) {
      ...
      if (request_content_type_toLower.includes('application/x-www-form-urlencoded') || request_content_type_toLower.includes('multipart/form-data') || request_content_type_toLower.includes('application/json')) {
    ...
    let original_response_copy = original_response;
    let response_headers = original_response_copy.headers;
    let response_status = original_response_copy.status;
    ...
    if (set_cookie_samesite_none && new_response_headers.has('set-cookie')) {
      ...
      new_response_headers.set('set-cookie', firstCookie.split('SameSite=Lax; Secure').join('').split('SameSite=Lax').join('').split('SameSite=Strict; Secure').join('').split('SameSite=Strict').join('').split('SameSite=None; Secure').join('').split('SameSite=None').join('').replace(/^;+$/g, '') + '; SameSite=None; Secure');
    }
    ...
    if (response_content_type && replacement_content_types.some((v) => response_content_type.toLowerCase().includes(v))) {
      original_text = await replace_response_text(original_response_copy, upstream_domain, url_hostname);
    } else {
      original_text = original_response_copy.body;
    }
